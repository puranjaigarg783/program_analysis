...
impl Cfg {
    ...
    pub fn new(function: &Function, globals: Set<VarId>, structs: Map<StructId, Set<FieldId>>) -> Self {
        ...
        */
        ...
        return_cfg.calculate_addr_takens_snk_src(&function.body);
    ...
    fn calculate_addr_takens_snk_src(&mut self, body: &Map<BbId, BasicBlock>) {
        ...
        for bb in body.values() {
            for inst in &bb.insts {
                match inst {
                    ...
                    Instruction::CallExt { lhs: _, ext_callee, args: _ } => {
                        if ext_callee.name().starts_with("src") {
                            self.srcs.insert(ext_callee.clone());
                            println!("inserted src: {}", ext_callee.name());
                        } else if ext_callee.name().starts_with("snk") {
                            self.snks.insert(ext_callee.clone());
                            println!("inserted snk: {}", ext_callee.name());
                        }
                    },
                    _ => (),
                }
...
pub trait AbstractEnv: Clone {
    ...
    fn analyze_term(&mut self, inst: &Terminal, cfg: &Cfg, soln: &mut Map<FuncId, FuncId>, store: &mut Map<VarId, Set<FuncId>>) -> Set<BbId>;
...
pub fn forward_analysis<A: AbstractEnv>(
    ...
    let mut store: Map<VarId, Set<FuncId>> = Map::new();
    // Initialize
